...
3. **Stage 4.2 — pandoc 转换**（在 `html_report/` 目录内执行，保证图片相对路径正确）：
   ...
   pandoc merged_mathml.html -f html -t docx --mathml --reference-doc="C:\Users\SUN\.gemini\antigravity\skills\doc-writer\references\EDAN_word模板.docx" -o "../<文档标题>.docx"
   ```
...
4. **⚠️ Stage 4.3 — 格式规范化后处理**：
   ```bash
    ...
    ```
    该脚本执行 **EDAN 专属样式映射**，并自动剥离 HTML 冗余编号。映射逻辑如下：

    | 元素类型 | HTML 标记 | Word 目标样式名 |
    | :--- | :--- | :--- |
    | **首页标题** | `h1` (第一个) | `文件名称` |
    | **正文总标题**| `h1` (后续) | `标题 1` |
    | **一级章节标题**| `h2` | `文章一级标题` |
    ...
    | **三级章节标题**| `h4` | `文章三级标题` |
    | **正文段落** | `p` | `文章正文段落` |
    | **图表题注** | `figcaption` / `caption` | `edan题注` |
    | **图片样式** | `img` | `图片样式` |
    | **表格正文** | `table` | `表格正文` |

    ⚠️ **编号剥离**：脚本会自动通过正则匹配移除标题开头的数字（如 `3. `、`3.1. `），以配合模板的自动多级列表功能。
...
| `scripts/apply_docx_format.py` | 对 DOCX 应用 EDAN 样式规范并剥离冗余编号 | 阶段 4 Stage 4.3 |

**脚本调用方式（均使用绝对路径）**：
```bash
# 脚本位于 skill 安装目录，调用时使用绝对路径：
# Windows: C:\Users\SUN\.gemini\antigravity\skills\doc-writer\scripts\<脚本名>.py
python "C:\Users\SUN\.gemini\antigravity\skills\doc-writer\scripts\extract_references.py" <文档根目录>
python "C:\Users\SUN\.gemini\antigravity\skills\doc-writer\scripts\latex_to_mathml.py" <merged.html路径> <output路径>
python "C:\Users\SUN\.gemini\antigravity\skills\doc-writer\scripts\apply_docx_format.py" <docx路径>
```
...
| `references/EDAN_word模板.docx` | EDAN Instruments 官方 Word 受控模板 | 阶段 4 Stage 4.2 |
